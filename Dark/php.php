...
$n=$_COOKIE['userdata'];
  if ($n=='Saja' && $n=='Arar'){
    header('location:../login.php');}
$s = "select * from user_courses where user_name= '$name' && cours_id = '3'";
...
<script>
  ...
  function en() 
  {
      ...
      var num = "<?php echo(json_encode($num)); ?>";
        if (num==0)
      {  
        location.href = 'enroll.php?c=3';
        
      }

      else {
        location.href = 'un.php?c=3';
      }
...
<div class="row">
<div class="col-md-6">
<p class="promo-title"> Learn PHP </p>
<p>Here you can the best way you want to learn php Just click on Enroll now to get it </p>
...
$fetchVideos = mysqli_query($con, "SELECT vid_location , vid_description ,time_added FROM course_video where course_id='3' ORDER BY `course_video`.`time_added` ASC");
while($row = mysqli_fetch_assoc($fetchVideos)){
 ...
 echo "<div>  <h6> added at :- </br> $t </h6> </div>";
...
<a class="nav-link" href="index.php"><button type="button" class="btn btn-primary">Back to main</button></a>
