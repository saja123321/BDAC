...
$n=$_COOKIE['userdata'];
  if ($n=='Saja' && $n=='Arar'){
    header('location:../login.php');}
$s = "select * from user_courses where user_name= '$name' && cours_id = '6'";
...
<script>
  ...
  function en() 
  {
      ...
      var num = "<?php echo(json_encode($num)); ?>";
        if (num==0)
      {  
        location.href = 'enroll.php?c=6';
        
      }

      else {
        location.href = 'un.php?c=6';
      }
...
<div class="row">
<div class="col-md-6">
<p class="promo-title"> Learn JavaScript </p>
<p>Here you can the best way you want to learn JavaScript Just click on Enroll now to get it </p>
...
$fetchVideos = mysqli_query($con, "SELECT vid_location , vid_description FROM course_video where course_id='6' ORDER BY `course_video`.`time_added` ASC");
